...
const Hero = () => {
  ...
  return (
    <section className="pt-28 md:pt-32 pb-16 md:pb-24 min-h-[90vh] md:min-h-screen flex flex-col items-center relative overflow-hidden">
      {/* Dark background base */}
      <div className="absolute inset-0 bg-[#0e0118] -z-30"></div>
      
      {/* Candle-like luminating effect */}
      <div 
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] h-[70vh] 
        rounded-full bg-purple-600/20 blur-[120px] animate-candlelight -z-20"
      />
      <div 
        className="absolute left-1/3 top-1/3 w-[50vw] h-[50vh] 
        rounded-full bg-purple-500/15 blur-[100px] animate-candlelight-delayed -z-20"
      />
      
      {/* Additional ambient glow */}
      <div 
        className="absolute right-1/4 bottom-1/4 w-[40vw] h-[40vh] 
        rounded-full bg-purple-700/20 blur-[90px] animate-candlelight-slow -z-20"
      />
      ...
      <div className="container mx-auto px-4 mb-6 md:mb-10">
        <div className="text-center max-w-5xl mx-auto">
          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-4 md:mb-6 leading-tight tracking-tight text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl md:text-4xl font-bold mb-3 md:mb-5 text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.25)]">
            You Never Have to Record Again.
          </h2>
          
          <p className="text-base md:text-lg text-gray-300 mb-6 md:mb-8 max-w-3xl mx-auto drop-shadow-[0_0_8px_rgba(200,200,255,0.15)]">
          ...
          <div className="flex justify-center mb-8 md:mb-10">
            <button 
              className="relative text-white text-base md:text-lg font-bold px-6 md:px-8 py-3 md:py-4 rounded-full transition-all duration-300 overflow-hidden group"
      ...
      <div className="w-full overflow-hidden">
        <div 
          ref={scrollRef} 
          className="flex space-x-3 md:space-x-4 phoneCarousel px-2 md:px-4 py-2"
        >
          {clientImages.map((image, index) => (
            <div key={`img-${index}`} className="flex-shrink-0 w-[160px] sm:w-[180px] md:w-[250px]">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent">
                  <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4">
                    <div className="transition-transform duration-300 group-hover:scale-105 group-hover:translate-y-[-4px]">
                      <div className="text-white font-bold text-base md:text-lg mb-0.5 md:mb-1 group-hover:text-purple-200 transition-colors duration-300">
          ...
          {clientImages.map((image, index) => (
            <div key={`img-dup-${index}`} className="flex-shrink-0 w-[160px] sm:w-[180px] md:w-[250px]">
              <div className="relative rounded-xl overflow-hidden aspect-[9/16] shadow-lg group">
                <img 
                  src={image} 
                  alt={`${clientInfo[index].name} - ${clientInfo[index].title}`} 
